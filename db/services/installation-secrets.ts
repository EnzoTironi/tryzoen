...
export async function resolvedInstallationSecrets() {
  const secrets = await getInstallationSecrets();
  return {
    betterAuthSecret: new Secret(secrets.betterAuthSecret),
    secretEncryptionKey: new Secret(secrets.secretEncryptionKey),
  };
}
